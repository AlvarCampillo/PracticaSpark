...
trait BatchJob {
  ...
  def computeUserQuotaLimit(dataFrame: DataFrame): DataFrame
  ...
  def run(args: Array[String]): Unit = {
    ...
    val aggByCoordinatesDF = computeBytesHourly(antennaMetadataDF)
    val aggErroAntennaDF = computeUserQuotaLimit(antennaMetadataDF)
